...
const SportsResult = () => {
    ...
    useEffect(() => {
        ...
        const unsubScores = onSnapshot(q, (snapshot) => {
            ...
            setLoading(false);
            // Cache for offline
            localStorage.setItem("sports_data", JSON.stringify(scoresData));
        }, (err) => {
            console.error("Error fetching live data:", err);
            setError(err.message);
            const cached = localStorage.getItem("sports_data");
    ...
    if (loading && data.length === 0) {
        return (
            <div className="flex justify-center items-center h-[50vh]">
                <span className="loading loading-ring loading-lg text-purple-500 scale-150"></span>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-12 animate-fade-in-up">
            ...
            {/* Leaderboard Section */}
            <div>
                <h2 className="text-3xl font-bold text-white mb-6 pl-2 border-l-4 border-purple-500">Overall Standings</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {groupScores.map((group, index) => (
                        <div key={group.id} className="relative glass-card rounded-2xl p-6 overflow-hidden transition-all hover:-translate-y-2 hover:shadow-purple-500/20">
                            ...
                            <div className="relative z-10 flex flex-col items-center">
                                ...
                                <div className="text-5xl font-black text-purple-400 mt-2">{group.score}</div>
            ...
            <div className="glass rounded-3xl p-1 md:p-8">
                <div className="flex justify-between items-center mb-6 px-4">
                    <h2 className="text-2xl font-bold text-white">Event Results</h2>
